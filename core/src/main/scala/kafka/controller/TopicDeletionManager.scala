...
class ControllerDeletionClient(controller: KafkaController, zkClient: KafkaZkClient) extends DeletionClient {
  ...
  // 删除/admin/delete_topics下的给定 topic 子节点
  override def deleteTopicDeletions(topics: Seq[String], epochZkVersion: Int): Unit = {
    //epochZkVersion表示controller的版本，为了防止僵尸controller的请求到zk，如果zk看到epochZkVersion小于自己的就可以拒绝。
    zkClient.deleteTopicDeletions(topics, epochZkVersion)
  }
  // 取消/brokers/topics/<topic>节点数据变更的监听，以防止其他用户对改主题的其他操作造成数据状态的不一致。
  ...
  private def resumeDeletions(): Unit = {
    ...
    topicsQueuedForDeletion.foreach { topic =>
      ...
      if (controllerContext.areAllReplicasInState(topic, ReplicaDeletionSuccessful)) {
        ...
        // 调用completeDeleteTopic方法完成后续操作即可，真正的删除主题
    ...
    // 调用 onTopicDeletion 方法，对待删除主题列表中的主题相关的分区执行删除操作
