...
class ControllerContext {
  ...
  private val liveBrokerEpochs = mutable.Map.empty[Int, Long]// Controller当前Epoch值
  var epoch: Int = KafkaController.InitialControllerEpoch// 运行中Broker Epoch列表
  ...
  val replicasOnOfflineDirs = mutable.Map.empty[Int, Set[TopicPartition]]// 已开启删除的主题列表

  val topicsToBeDeleted = mutable.Set.empty[String]// 暂时无法执行删除的主题列表
