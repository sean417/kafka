...
abstract class AbstractFetcherThread(name: String,
  ...
  protected def truncate( // 要对哪个分区下副本执行截断操作
  ...
  //当分区存在 Leader Epoch 值时，源码会将副本的本地日志截断到 Leader Epoch 对应的最新位移值处，
  // 即方法 truncateToEpochEndOffsets 的逻辑实现；相反地，如果分区不存在对应的 Leader Epoch 记录，
  // 那么依然使用原来的高水位机制，调用方法 truncateToHighWatermark 将日志调整到高水位值处。
  ...
  private def processFetchRequest(sessionPartitions: util.Map[TopicPartition, FetchRequest.PartitionData],
    ...
    try {
      ...
      // 给Leader发送FETCH请求，阻塞的等待response返回
